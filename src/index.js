...
export default class BackgroundGeofence {
  ...
  static addListener(event, success, failure) {
    if (typeof event != 'string') {
      throw 'BackgroundGeofence#on must be provided a {String} event as 1st argument.';
    }
    if (Events[event]) {
      throw "BackgroundGeofence#on - Unknown event '" + event + "'";
    }
    if (typeof success != 'function') {
      throw "BackgroundGeofence#on must be provided a callback as 2nd argument.  If you're attempting to use the Promise API to listen to an event, it won't work, since a Promise can only evaluate once, while the callback function must be executed for each event.";
    }
  ...
  static removeListener(event, handler, success, failure) {
    if (typeof event != 'string') {
      throw 'BackgroundGeolocation#un must be provided a {String} event as 1st argument';
    }
    if (!Events[event.toUpperCase()]) {
      throw "BackgroundGeolocation#un - Unknown event '" + event + "'";
    }
  ...
  static startBackgroundTask(success, failure) {
    if (!arguments.length) {
      ...
      if (typeof success !== 'function') {
        throw (
          TAG +
          '#startBackgroundTask must be provided with a callback to recieve the taskId'
        );
      }
  ...
  static watchPosition(success, failure, options) {
    if (typeof success === 'undefined') {
      throw (
        TAG +
        '#watchPosition cannot use Promises since a Promise can only evaluate once while the supplied callback must be executed for each location'
      );
    }
  ...
  static getLog(success, failure) {
    console.warn(
      '[' +
        TAG +
        ' getLog] Deprecated.  Use BackgroundGeolocation.logger.getLog'
    );
    if (!arguments.length) {
  ...
  static destroyLog(success, failure) {
    console.warn(
      '[' +
        TAG +
        ' destroyLog] Deprecated.  Use BackgroundGeolocation.logger.destroyLog'
    );
    if (!arguments.length) {
  ...
  static emailLog(email, success, failure) {
    console.warn(
      '[' +
        TAG +
        ' emailLog] Deprecated.  Use BackgroundGeolocation.logger.emailLog'
    );
    if (typeof email != 'string') {
      throw TAG + '#emailLog requires an email address as 1st argument';
    }
    if (arguments.length == 1) {
